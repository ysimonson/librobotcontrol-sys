# Use old version of alpine that's on musl v1.1
FROM --platform=linux/arm/v7 alpine:3.12 AS build
RUN apk update && apk add --no-cache binutils cmake make libgcc linux-headers musl-dev gcc g++ clang clang-dev
